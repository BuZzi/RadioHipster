...
/**
 * Base class that represents a row from the 'sort' table.
 *
 *
 *
 * @package    propel.generator.RadioHipster.om
 */
abstract class BaseSort extends BaseObject implements Persistent
{
    /**
     * Peer class name
     */
    ...
    /**
     * The value for the sort_name field.
     * @var        string
     */
    protected $sort_name;
    ...
    /**
     * Get the [sort_name] column value.
     *
     * @return string
     */
     ...
     * @return Sort The current object (for fluent API support)
     */
    public function setSortId($v)
    {
        ...
        if ($this->sort_id !== $v) {
            $this->sort_id = $v;
            $this->modifiedColumns[] = SortPeer::SORT_ID;
        }
    ...
    /**
     * Set the value of [sort_name] column.
     *
     * @param string $v new value
     * @return Sort The current object (for fluent API support)
     */
    public function setSortName($v)
    {
        if ($v !== null && is_numeric($v)) {
            $v = (string) $v;
        }
        ...
        if ($this->sort_name !== $v) {
            $this->sort_name = $v;
            $this->modifiedColumns[] = SortPeer::SORT_NAME;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->sort_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
            $this->sort_name = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
            $this->resetModified();
            ...
            return $startcol + 2; // 2 = SortPeer::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException("Error populating Sort object", $e);
        }
    }
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        $stmt = SortPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    public function save(PropelPDO $con = null)
    {
        ...
        try {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                SortPeer::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
            $con->commit();
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        $this->modifiedColumns[] = SortPeer::SORT_ID;
        if (null !== $this->sort_id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . SortPeer::SORT_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(SortPeer::SORT_ID)) {
            $modifiedColumns[':p' . $index++]  = '`sort_id`';
        }
        if ($this->isColumnModified(SortPeer::SORT_NAME)) {
            $modifiedColumns[':p' . $index++]  = '`sort_name`';
        }

        $sql = sprintf(
            'INSERT INTO `sort` (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case '`sort_id`':
                        ...
                        break;
                    case '`sort_name`':
                        $stmt->bindValue($identifier, $this->sort_name, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    protected function doValidate($columns = null)
    {
        if (!$this->alreadyInValidation) {
            ...
            if (($retval = SortPeer::doValidate($this, $columns)) !== true) {
    ...
    {
        $pos = SortPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['Sort'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Sort'][$this->getPrimaryKey()] = true;
        $keys = SortPeer::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getSortId(),
            $keys[1] => $this->getSortName(),
        );
    ...
    public function fromArray($arr, $keyType = BasePeer::TYPE_FIELDNAME)
    {
        ...
        if (array_key_exists($keys[0], $arr)) $this->setSortId($arr[$keys[0]]);
        if (array_key_exists($keys[1], $arr)) $this->setSortName($arr[$keys[1]]);
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SortPeer::SORT_ID)) $criteria->add(SortPeer::SORT_ID, $this->sort_id);
        if ($this->isColumnModified(SortPeer::SORT_NAME)) $criteria->add(SortPeer::SORT_NAME, $this->sort_name);
    ...
    {
        $criteria = new Criteria(SortPeer::DATABASE_NAME);
        $criteria->add(SortPeer::SORT_ID, $this->sort_id);
    ...
    /**
     * Generic method to set the primary key (sort_id column).
     *
     ...
     * objects.
     *
     * @param object $copyObj An object of Sort (or compatible) type.
    ...
    {
        $copyObj->setSortName($this->getSortName());
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setSortId(NULL); // this is a auto-increment column, so set to default value
        }
    }
     ...
     *
     * @param boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return Sort Clone of current object.
     * @throws PropelException
     */
     ...
     * If this Sort is new, it will return
     ...
     * Otherwise if this Sort is new, it will return
     * an empty collection; or if this Sort has previously
     * been saved, it will retrieve related Songs from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Sort.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
    ...
    {
        return (string) $this->exportTo(SortPeer::DEFAULT_STRING_FORMAT);
    }
